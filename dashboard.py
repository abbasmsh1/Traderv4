...
if 'trading_system' not in st.session_state:
    st.session_state.trading_system = TradingSystem(initial_balance_usd=100.0)
...
trade_amount = st.sidebar.number_input("Trade Amount (USD)", min_value=1.0, max_value=100.0, value=10.0)
col1, col2 = st.sidebar.columns(2)
if col1.button("Buy"):
    st.session_state.trading_system.execute_trade(symbol, 'buy', trade_amount)
if col2.button("Sell"):
    st.session_state.trading_system.execute_trade(symbol, 'sell', trade_amount)

# Auto-trading toggle
st.sidebar.subheader("Auto Trading")
auto_trade = st.sidebar.checkbox("Enable Auto Trading")
if auto_trade:
    trade_interval = st.sidebar.slider("Trading Interval (minutes)", 1, 60, 5)
